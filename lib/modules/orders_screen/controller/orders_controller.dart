...
import 'package:nourish_now/models/address_model.dart';
import 'package:nourish_now/models/cart_model.dart';
import 'package:nourish_now/models/order_model.dart';
...
class OrdersController extends GetxController {
  ...
  CollectionReference ordersRef = FirebaseFirestore.instance.collection("users")
      .doc(AppConstants.uid).collection('orders');
  ...
  Future<void> getOrders() async{
    ...
    await FirebaseFirestore.instance
      ...
      for (var element in value.docs) {
        if(element.data()['state'].toLowerCase()=="pending"||element.data()['state'].toLowerCase()=="in process") {
    ...
    ordersRef.snapshots()
      ...
      for (var change in querySnapshot.docChanges) {
        if (change.type == DocumentChangeType.modified) {
          OrderModel order = OrderModel.fromJson(change.doc.data()! as Map<String,dynamic>);
          if (order.state.toLowerCase() == 'in process') {
            runningOrders.firstWhere((element) => element.time==order.time).state=order.state;
          }
          ...
          Get.snackbar("Order Status", "Check Your Orders!!",
              colorText: Colors.green);
          sendNotification(title:"nourish_now" ,myBody: "Your order status has been changed to ${order.state}");
          update();
        }
  ...
  Future<void> removeOrder(OrderModel model) async{
    ...
    await FirebaseFirestore.instance
        .collection("users")
        .doc(AppConstants.uid)
        .collection('orders').where('time', isEqualTo: model.time).get()
        .then((value) async {
      for (var element in value.docs) {
