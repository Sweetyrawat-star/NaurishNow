...
Widget orderItems (OrderModel order,context, { bool inHistory=false}) {
  ...
  child: Column(
    ...
    children: [
      Row(
        children: [
          Text(DateFormat(inHistory?'dd/MM/yyyy hh:mm a':'hh:mm a').format(DateTime.parse(order.time)),style: const TextStyle(fontSize:18),),
          const Spacer(),
      ...
      !inHistory?SizedBox(height: Dimensions.height10,):Container(),
      SizedBox(
        ...
        child: Row(
          children: [
            ...
            Column(
              ...
              children: [
                inHistory?SizedBox(height: Dimensions.height10,):Container(),
                Text( 'Total',style: TextStyle(fontSize: Dimensions.height10*1.8),),
               const Spacer(),
               Text( '${order.items.length} Items'),
                const Spacer(),
                order.state.toLowerCase()!="in process"?CustomButton(onPressed: (){
                  if(inHistory){
                ...
                },transparent: true,buttonText: inHistory?"one more":"Cancel",height: Dimensions.height10*3,width: Dimensions.width10*9, ):Container()
              ],
            ),
          ],
        ),
